...
console.log('\n── 5. Lo que NO puede pasar ──')
{
  ...
  // El recuento con el que acaba el export de TCG Live tiene la MISMA
  // forma que una cabecera («palabras: número»), y no se puede enseñar
  // como línea ilegible: el editor lo convertía en error y no dejaba
  // guardar un export intacto (cambio de IBAI, 2026-09-11). Lo que lo
  // distingue de una cabecera desconocida es la palabra del TOTAL.
  const total = (linea) => decklistUnparsed(`Pokémon: 1\n1 Dreepy TWM 128\n${linea}`).length === 0
  check('el recuento final no se enseña como error', total('Total Cards: 60'))
  check('en español, declinado', total('Cartas totales: 60'))
  check('en francés', total('Nombre total de cartes: 60'))
  check('y en alemán', total('Karten gesamt: 60'))
  // Pero una cabecera de un idioma que el motor no entiende SÍ se
  // declara: si no, sus cartas desaparecen sin una palabra.
  check('una cabecera desconocida sigue declarándose', !total('Sección Rara: 1') && !total('Trainerkaarten: 32'))
}
